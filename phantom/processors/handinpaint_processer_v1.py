...
class RobotInpaintProcessor(BaseProcessor):  
    ...
    def process_one_demo(self, data_sub_folder: str) -> None:
        ...
        - Visuals: Rendered using transformed coordinates (Arx5 specific logic).
        - Sequence: Saved using STANDARD RAW data (Standard logic).
        - Verification: PASSED (Dots align with robot).
        """
        ...
        if self.robot == "Arx5":
            try:
                ...
                if not os.path.exists(xml_path_str): return
                dual_robot = DualArmController(xml_path_str, max_steps=100)

                # 2. 准备渲染数据 (Visuals Pipeline) - 仅用于生成视频
                # -----------------------------------------------------
                # 坐标系定义
                ...
                seqs_L_in_world = np.array([matrix_to_pose(m) for m in Mat_world_T_seqs_L])
                seqs_R_in_world = np.array([matrix_to_pose(m) for m in Mat_world_T_seqs_R])
                camera_poses_world = np.tile(matrix_to_pose(Mat_world_T_base_L @ Mat_base_L_T_camera), (len(seqs_L_in_world), 1))
                ...
                sim_frames, sim_masks = dual_robot.move_trajectory_with_camera(
                    seqs_L_in_world[:min_frames], seqs_R_in_world[:min_frames], camera_poses_world[:min_frames], 
                    50, kinematic_only=True, cam_name="camera", width=640, height=480
                )
                if len(sim_frames) == 0: return

                # 3. 准备标签数据 (Labels Pipeline) - 用于 Sequence
                # -----------------------------------------------------
                # [VERIFIED Correct]: 使用标准原始数据，不经过任何仿真变换
                print("Loading STANDARD RAW DATA for Sequence Labeling...")
