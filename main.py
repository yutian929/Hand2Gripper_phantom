...
class RGBD_VO:
    ...
    """
    def __init__(self, rgb_video_path, depth_npy_path, camera_matrix):
        """
        初始化RGB-D视觉里程计。
...
def test_rgbd_vo_with_orb():
    # 1. 定义路径和相机参数
    # !!! 请务必修改为您的真实文件路径 !!!
    # rgb_video_path = "/home/yutian/projs/Hand2Gripper_phantom/output/video.mp4"
    rgb_video_path = "/home/yutian/projs/Hand2Gripper_phantom/data/raw/pick_and_place/0/video_L.mp4"
    # 假设深度图已保存为npy文件，且与视频帧一一对应
    # depth_npy_path = "/home/yutian/projs/Hand2Gripper_phantom/output/depth.npy" 
    depth_npy_path = "/home/yutian/projs/Hand2Gripper_phantom/data/raw/pick_and_place/0/depth.npy" 
    ...
    if not os.path.exists(depth_npy_path):
        ...
        video_frames = media.read_video(rgb_video_path)
        # 创建一个所有点深度都为2米的假深度数据
        fake_depth = np.ones((len(video_frames), video_frames.shape[1], video_frames.shape[2]), dtype=np.float32) * 2.0
        np.save(depth_npy_path, fake_depth)
        print(f"已创建假的深度文件: '{depth_npy_path}'")
    ...
    # 2. 初始化并运行VO
    vo = RGBD_VO(rgb_video_path, depth_npy_path, K)
    trajectory = vo.run()

    # 3. 打印并可视化结果
    print("\n相机运动轨迹 (X, Y, Z):")
    print(trajectory)
    RGBD_VO.plot_trajectory(trajectory)
...
if __name__ == "__main__":
    # test_python_orb_slam3()
    # test_rgbd_vo_with_orb()
    test_robo_suite()